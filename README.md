...
| Model | Classes | Backbone | Final Accuracy |
|---|---|---|---|
| Coffee (Full) | 15 coffee drinks | MobileNetV2 | 36% |
| Bikes | Unicycle / Bicycle / Tricycle | MobileNetV2 | 90% |
| Rainbows | No Rainbow / Rainbow / Double Rainbow | ViT-Base | 87% |
| Coffee (Reduced) | 7 coffee drinks | CLIP ViT-B/32 | ~70%+ |
...
**Code:** [`model-experiment-coffee-full/`](model-experiment-coffee-full/)
...
**Code:** [`model-experiment-bikes/`](model-experiment-bikes/)
...
**Code:** [`model-experiment-rainbows/`](model-experiment-rainbows/)
...
**Total runtime:** ~35 minutes. Cost tracked with `ccusage`.
...
**Code:** [`model-experiment-coffee-reduced/`](model-experiment-coffee-reduced/)
...
Each experiment is self-contained with its own `requirements.txt`.

```bash
# Example: rainbows experiment
cd model-experiment-rainbows
pip install -r requirements.txt

# Generate model weights
python generate_weights.py

# Calibrate using example images
...
The `.bin` model files are not tracked in git (see `.gitignore`). Run the build/generate scripts to produce them.
